...
- 2026-05-14: deploy planning — picked DigitalOcean Droplet in Bangalore (TimescaleDB constraint rules out most managed Postgres; $200 Education credit covers ~10 months of $20/mo droplet; ~30ms India latency). Wrote `docs/deploy-digitalocean.md` (13-section step-by-step runbook: provisioning → SSH lockdown → Docker install → GHCR auth → first deploy → nginx + Let's Encrypt → daily Postgres dump backups → uptime monitoring → troubleshooting → scale-out triggers) + `apps/server/docker-compose.prod.yml` (pinned image tags, host-volume mounts for backup-capture, depends_on:condition:service_healthy, localhost-only port binding). CLAUDE.md pointer block updated.
