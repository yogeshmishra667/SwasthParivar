...
**Last updated:** 2026-05-15 (session 8 — step 3d `ANALYZE_READING` worker)
**Status:** ✅ **Phase 1 complete.** 🚧 **Phase 2 in progress** — main merged steps 0+1; steps 2 + 3a + 3b + 3c + 3d shipped as PRs awaiting review.
**Branch:** main (9 Phase 1 PRs + #14 #15 #16 merged) + open Phase 2 PRs: `feat/meal-logs-server` (#17, step 2 + docs), `feat/insight-engine-foundation` (#18, step 3a), `feat/insight-detectors-spike-trend` (#19, step 3b), `feat/insight-detectors-meal-anomaly` (step 3c — `detectMealCorrelation` + `detectAnomaly` + step 3d — `ANALYZE_READING` worker).
...
4. 🚧 **Step 3 — Insight engine** — split into 3 sub-PRs to keep diff ≤ 600 lines each:
   ...
   - ✅ **3c — `detectMealCorrelation` + `detectAnomaly`** pure functions — branch `feat/insight-detectors-meal-anomaly`
   - ✅ **3d — `ANALYZE_READING` BullMQ worker** wiring (runs all 4 detectors post-glucose-insert, persists InsightEvent rows). Same branch — bundled with 3c.
...
- [x] **Step 3c — `detectMealCorrelation` + `detectAnomaly`** pure functions in `packages/domain-logic/src/detectors/`. **Meal-correlation**: 7-calendar-day window, min 5 post_meal readings per category, attributes a reading to the most-recent meal logged within the prior 3 hours, picks the category with the largest delta vs overall baseline, severity bands info(+10) / warn(+25) / critical(+50) mg/dL above baseline. **Anomaly**: 21-day window, same-readingType, Tukey fences (Q1 - 1.5*IQR, Q3 + 1.5*IQR) flagging mild outliers (warn) and extreme outliers (≥ 3\*IQR → critical). Direction tagged high/low. Constant baseline (IQR=0) returns null. 22 new tests (18 cases + 2 fast-check property tests + 2 baseline-isolation tests). Detector files: **100% lines + 100% functions, 92.76% branches** across `meal-correlation.ts` + `anomaly.ts` + earlier 3a/3b detectors. Domain-logic suite: **126/126 ✅**. New `MealEntry` domain-logic type added (mirrors `MealLog` Prisma model). Branch `feat/insight-detectors-meal-anomaly`.
- [x] **Step 3d — `ANALYZE_READING` BullMQ worker** wiring. Fires post-glucose-insert via `analyzeQueue.add("analyze", ...)` in `readings.service.ts`, after the critical-bypass enqueue path. New files `apps/server/src/workers/analyze-reading.processor.ts` (pure-import target for tests) + `analyze-reading.worker.ts` (side-effecting `createWorker` binding) mirror the critical-alert split. Processor reads 35 days of glucose + 7 days of meals in parallel, maps Prisma rows → `TypedReading`/`MealEntry`, runs all 4 detectors in parallel via `Promise.all`, then `prisma.insightEvent.createMany` for every non-null `DetectorResult`. Below-floor (confidence < 0.7) rows persist; the 3a feed filter already suppresses them. Worker registered in `workers/index.ts` (`analyzeReadingWorker`, added to `workers[]` + `workerNames`). 3 retries + exp backoff come from `shared/queue.ts` defaults. `pnpm -r typecheck` + targeted lint on touched files + manual purity grep (no `@prisma`/`bullmq`/`ioredis` in `packages/domain-logic`) + domain-logic **126/126** all green.
