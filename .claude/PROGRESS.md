...
**Status:** ✅ **Phase 1 complete.** 🚧 **Phase 2 in progress** — main merged steps 0+1; steps 2 + 3a + 3b + 3c shipped as PRs awaiting review. Step 3d (`ANALYZE_READING` worker wiring) deferred until PR #17 merges.
**Branch:** main (9 Phase 1 PRs + #14 #15 #16 merged) + open Phase 2 PRs: `feat/meal-logs-server` (#17, step 2 + docs), `feat/insight-engine-foundation` (#18, step 3a), `feat/insight-detectors-spike-trend` (#19, step 3b), `feat/insight-detectors-meal-anomaly` (step 3c — `detectMealCorrelation` + `detectAnomaly`).
...
4. 🚧 **Step 3 — Insight engine** — split into 3 sub-PRs to keep diff ≤ 600 lines each:
   ...
   - ⏭️ **3d — `ANALYZE_READING` BullMQ worker** wiring (runs all 4 detectors post-glucose-insert, persists InsightEvent rows). Gated on PR #17 merging — the worker queries `meal_logs`, which only lands on main when #17 lands.
...
- [ ] **Step 3d — `ANALYZE_READING` BullMQ worker** wiring. Fires post-glucose-insert (3 retries, exp backoff). Runs all 4 detectors in parallel; below-confidence-floor results persist with `confidence < 0.7` (suppressed from feed); above-floor results visible. Maps `MealLog` Prisma rows → `MealEntry` shape at the service boundary so the detector stays pure. **Gated on PR #17 (MealLog) merging** — the worker queries `meal_logs`, which only exists on main when #17 lands.
